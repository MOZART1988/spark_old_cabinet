...
class SendOrder implements ShouldQueue
{
	...
	public function __construct($leads,$id,$noteText,$template_id)
	{
		$this->leads = $leads;
		$this->id = $id;
		  ...
		  #Если код ответа не равен 200 или 204 - возвращаем сообщение об ошибке
			if($code!=200 && $code!=204) {
				...
				DB::table('notifications')
					->insert([
						'userid'    =>  $this->id,
						'text'      =>  'Заявка успешно принята. Для печати накладных нажмите <a href="https://cabinet.spark-logistics.kz/waybills.html?id='.$this->template_id.'">сюда</a>.'
					]);
				$job = (new SendNote(json_decode($out)->_embedded->items[0]->id, $this->noteText))->onQueue('notes');
				dispatch($job);				
