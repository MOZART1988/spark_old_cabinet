...
class getContragents implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $subdomain = 'sparkcrm'; #Наш аккаунт - поддомен
        #Формируем ссылку для запроса
        $link = 'https://' . $subdomain . '.amocrm.ru/api/v2/companies?query='.$this->name;
 ...
 */
        ...
        foreach ($Response as $v) {
            if (is_array($v)) {
                $output = $v['id'];
                file_put_contents('/var/www/html/API/companyIDs.txt', $output."\n", FILE_APPEND);
            }
        }
